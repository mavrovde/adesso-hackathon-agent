...
1. **Coordinator Agent:** (Implemented in [coordinator.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/coordinator.py)) Initiates the triage loop, classifies incoming requests by category and priority (P1–P4), enriches them with user metadata, and manages the orchestration flow.
...
3. **Agent Loop with `stop_reason` Handling:** A custom main loop (initiated from [main.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/main.py)) wraps the agent execution to handle Claude's tool usage, token exhaustion, and turn completions deterministically.
...
5. **Deterministic PreToolUse Hook & Escalation Rules:** Hard boundaries (e.g., prohibiting password resets on frozen accounts) are enforced deterministically at the system hook level (implemented in [hooks.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/hooks.py)), while soft rules (confidence, dollar impact) are evaluated at the coordination level.
...
- **`stop_reason == "tool_use"`:** The coordinator intercepts the tool call. The `PreToolUse` hook (in [hooks.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/hooks.py)) executes deterministic checks (e.g., PII extraction patterns, frozen accounts status). If blocked, it returns a block dictionary `{"block": True, "reason": "..."}` which triggers an immediate stop and escalation. If allowed, the tool is executed and results are returned to Claude.
...
- **Location:** Implemented in [password_reset.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/specialists/password_reset.py)
- **Scope:** Auto-resolution of password reset tickets for verified, active users.
- **Tools (4):**
  1. `get_user_context(user_id)`: (in [get_user_context.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/get_user_context.py)) Verifies role, account status, and history.
  2. `lookup_kb(query)`: (in [lookup_kb.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/lookup_kb.py)) Finds system-specific reset runbooks.
  3. `reset_user_password(user_id, system)`: (in [reset_user_password.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/reset_user_password.py)) Performs the secure reset.
  4. `resolve_ticket(ticket_id, resolution_summary)`: (in [resolve_ticket.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/resolve_ticket.py)) Closes the ticket with an audit trail.

#### B. IT Specialist Agent (`ITSpecialistAgent`)
- **Location:** Implemented in [it_specialist.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/specialists/it_specialist.py)
...
- **Tools (5):**
  1. `get_user_context(user_id)`: (in [get_user_context.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/get_user_context.py)) Fetches user profile metadata.
  2. `lookup_kb(query)`: (in [lookup_kb.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/lookup_kb.py)) Searches for known-error runbooks.
  3. `create_or_update_ticket(...)`: (in [create_or_update_ticket.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/create_or_update_ticket.py)) Enriches and writes classifications to the ticketing system.
  4. `resolve_ticket(ticket_id, resolution_summary)`: (in [resolve_ticket.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/resolve_ticket.py)) Closes the ticket.
  5. `escalate(ticket_id, reason, severity, impact)`: (in [escalate.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/escalate.py)) Triggers human handoff.
...
- **Strict Guardrails:** Deterministic `PreToolUse` hooks (such as checking `PROMPT_INJECTION_PATTERNS` or account status from the [mock_store.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/mock_store.py)) guarantee that no model-based hallucination or injection can trigger unauthorized account lockouts or VIP modifications.
- **Graceful Error Recovery:** Structured tool responses (defined in [mock_store.py](file:///Users/sergii.mavrov/Workshop/adesso-hackathon-agent/agent/tools/mock_store.py) and each individual tool) allow the model to read error codes programmatically and attempt alternative strategies (e.g., query refinement) rather than failing on raw stack traces.
