...
1. **Coordinator Agent:** Initiates the triage loop, classifies incoming requests by category and priority (P1–P4), enriches them with user metadata, and manages the orchestration flow.
...
3. **Agent Loop with `stop_reason` Handling:** A custom main loop wraps the agent execution to handle Claude's tool usage, token exhaustion, and turn completions deterministically.
...
5. **Deterministic PreToolUse Hook & Escalation Rules:** Hard boundaries (e.g., prohibiting password resets on frozen accounts) are enforced at the system hook level, while soft rules (confidence, dollar impact) are evaluated at the coordination level.
...
- **`stop_reason == "tool_use"`:** The coordinator intercepts the tool call. The `PreToolUse` hook executes deterministic checks (e.g., PII extraction patterns, frozen accounts status). If blocked, it stops and escalates. If allowed, the tool is executed and results are returned to Claude.
...
- **Tools (4):**
  1. `get_user_context(user_id)`: Verifies role, account status, and history.
  2. `lookup_kb(query)`: Finds system-specific reset runbooks.
  3. `reset_user_password(user_id, system)`: Performs the secure reset.
  4. `resolve_ticket(ticket_id, resolution_summary)`: Closes the ticket with an audit trail.

#### B. Incident Specialist (`IncidentSpecialist`)
...
- **Tools (5):**
  1. `get_user_context(user_id)`: Fetches user profile metadata.
  2. `lookup_kb(query)`: Searches for known-error runbooks.
  3. `create_or_update_ticket(...)`: Enriches and writes classifications to the ticketing system.
  4. `resolve_ticket(ticket_id, resolution_summary)`: Closes the ticket.
  5. `escalate(ticket_id, reason, severity, impact)`: Triggers human handoff.
...
- **Strict Guardrails:** Deterministic `PreToolUse` hooks guarantee that no model-based hallucination or injection can trigger unauthorized account lockouts or VIP modifications.
- **Graceful Error Recovery:** Structured tool responses allow the model to read error codes programmatically and attempt alternative strategies (e.g., query refinement) rather than failing on raw stack traces.
